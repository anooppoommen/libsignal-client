...
fn bridge_fn_body(
    ...
    quote! {
        ...
        match TransformHelper(__result).ok_if_needed() {
            ...
            Err(failure) => {
                let module = cx.this();
                node::SignalNodeError::throw(failure, &mut cx, module)
            }
        }
    }
}
        ...
        Ok(signal_neon_futures::promise(
            ...
            std::panic::AssertUnwindSafe(async move {
                ...
                signal_neon_futures::settle_promise(move |cx| {
                    ...
                    match __result {
                        Ok(success) => Ok(node::ResultTypeInfo::convert_into(success, *cx)?.upcast()),
                        Err(failure) => node::SignalNodeError::throw(failure, *cx, __this),
                    }
                })
            })
        )?.upcast())
